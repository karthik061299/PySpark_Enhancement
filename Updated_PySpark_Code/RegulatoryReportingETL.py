...
# Date: 
# Description: PySpark ETL for Regulatory Reporting - generates AML_CUSTOMER_TRANSACTIONS and enhanced BRANCH_SUMMARY_REPORT with branch-level operational metadata integration.
# Updated by: ASCENDION AVA+
# Updated on: 
# Update Description: Integrated BRANCH_OPERATIONAL_DETAILS into BRANCH_SUMMARY_REPORT; added REGION and LAST_AUDIT_DATE fields with conditional logic per technical specification; enhanced documentation and traceability.
# ====================================================================
...
# [MODIFIED] Enhanced function to integrate BRANCH_OPERATIONAL_DETAILS and conditional logic for REGION and LAST_AUDIT_DATE
# [ADDED] branch_operational_details_df as input
...
def create_branch_summary_report(transaction_df: DataFrame, account_df: DataFrame, branch_df: DataFrame, branch_operational_details_df: DataFrame) -> DataFrame:
    ...
    :param branch_operational_details_df: DataFrame with branch operational details.
    :return: A DataFrame containing the branch summary report with REGION and LAST_AUDIT_DATE.
    """
    ...
    summary_df = summary_df.join(branch_operational_details_df, "BRANCH_ID", "left")
    # [ADDED] Conditional population of REGION and LAST_AUDIT_DATE
    summary_df = summary_df.withColumn(
        "REGION", when(col("IS_ACTIVE") == 'Y', col("REGION")).otherwise(None)  # [MODIFIED] Only active branches
    ).withColumn(
        "LAST_AUDIT_DATE", when(col("IS_ACTIVE") == 'Y', col("LAST_AUDIT_DATE")).otherwise(None)  # [MODIFIED]
    )
    # [DEPRECATED] The following columns are no longer in the final output but kept for traceability:
    # summary_df = summary_df.drop("MANAGER_NAME", "IS_ACTIVE")  # [DEPRECATED] Dropped as per new requirements
...
def main():
    ...
    try:
        ...
        branch_operational_details_df = read_table(spark, jdbc_url, "BRANCH_OPERATIONAL_DETAILS", connection_properties)  # [ADDED]
        ...
        # Create and write BRANCH_SUMMARY_REPORT with new logic
        branch_summary_df = create_branch_summary_report(
            transaction_df, account_df, branch_df, branch_operational_details_df  # [MODIFIED]
        )
